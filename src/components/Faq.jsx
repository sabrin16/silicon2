...
const Faq = () => {
  ...
  useEffect(() => {
    fetch("https://win24-assignment.azurewebsites.net/api/faq")
      ...
      .then((data) => {
        console.log("Fetched FAQ Items:", data);
        setFaqItems(data);
        setLoading(false);
      })
      .catch((error) => {
        console.error("Error fetching FAQ:", error);
        setError("Det gick inte att hämta FAQ-data: " + error.message);
        setLoading(false);
      });
  }, []);
        ...
        <div className="accordion-body">
          <div className="accordion">
            {faqItems.map((item, index) => (
              <div
                key={index}
                className={`container ${openIndex === index ? "active" : ""}`}
              >
                <div className="accordion-info">
                  <h4 className="label">{item.title}</h4>
                  <div
